...
class ShowGradCam:
    ...
    # def gen_cam(self, feature_map, grads, fc_weights):
    #     """
    #     依据梯度和特征图，生成cam
    #     :param feature_map: np.array， in [C, H, W]
    #     :param grads: np.array， in [C, H, W]
    ...
    #     # for i, w in enumerate(feature_map):
    #         # cam += w * feature_map[i, :, :]
    #     # print(feature_map.squeeze(0))
    #     print(fc_weights[1, :])
    #     # exit(0)
    #     cam = (feature_map.squeeze(0) * fc_weights[:, 0]).sum(dim=0)
    #     cam = cam.cpu().data.numpy()
    #     cam = np.maximum(cam, 0)
    #     # cam = np.maximum(feature_map[1, :, :], 0) * fc_weights
    ...
    # def show_on_img(self, input_img, cnt, fc_weights):
    #     '''
    #     write heatmap on target img
    #     :param input_img: cv2:ndarray/img_pth
    #     :return: save jpg
    ...
    #     fmap = self.feature_res[0]#.cpu().data.numpy().squeeze()
    #     grads_val = self.grad_res[0].cpu().data.numpy().squeeze()
    #     cam = self.gen_cam(fmap, grads_val, fc_weights)
    #     cam = cv2.resize(cam, img_size)
    #     heatmap = cv2.applyColorMap(np.uint8(255 * cam), cv2.COLORMAP_JET)#/255.
    #     # cam = heatmap + np.float32(input_img/255.)
    #     # cam = cam / np.max(cam)*255
